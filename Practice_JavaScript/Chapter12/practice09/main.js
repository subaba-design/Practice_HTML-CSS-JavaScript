...
// ここにcreateTodo関数を作る
function createTodo() {
    const createdTodo = prompt('タスクを入力してください');
    if (!createdTodo) {
...
function deleteTodo() {
    ...
    let deleteTargetNumber = parseInt(input, 10);
    if (isNaN(deleteTargetNumber) || deleteTargetNumber >= todos.length || deleteTargetNumber < 0) {
        alert('不正な値のためスキップします');
    } else {
        let deletedTodos = todos.splice(deleteTargetNumber, 1);
