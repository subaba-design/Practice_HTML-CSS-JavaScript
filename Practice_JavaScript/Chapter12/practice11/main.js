...
const removeMaxNumberFromArray = (_numbers) => {
    const resultNumbers = _numbers.slice();
    let removeTargetIndex = 0;
    // ↑元々は何の定義もしていなかったが、配列に要素がひとつだけだった場合、その要素を消すことができない。その対策として初期値は0とするべき
    resultNumbers.forEach((_number, index) => {
        if (_number < _numbers[index + 1]) {
            removeTargetIndex = index + 1;
        }
    });
    resultNumbers.splice(removeTargetIndex, 1);
...
console.log('returnedNumbers? : ', returnedNumbers);
